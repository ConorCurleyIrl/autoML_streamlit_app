...
from pycaret.classification import setup, compare_models, pull, save_model, load_model, predict_model, plot_model, tune_model
#conda install conda-forge::streamlit-pandas-profiling
...
if choice == 'Step1: Find your data and upload it!':
    ...
    # Initialization of variable into session state
...
if choice == 'Step2: Make me some pretty graphs!':
    ...
    #st.info('This datsaset has ' + str(st.session_state['df'].shape[0]) + ' rows and ' + str(st.session_state['df'].shape[1]) + ' columns.')
    ...
    if st.button(':blue[Make those pretty graphs for me!]') == True:
        ...
        st.session_state['profile'] = ProfileReport(st.session_state['df'], title='Profiling Report', explorative=True)
...
if choice == 'Step3: Machine Learning Time':
    ...
    target = st.selectbox("Select your Target Variable", st.session_state['df'].columns)
    ...
    st.session_state['temp_df']=st.session_state['df']
    st.session_state['temp_df']=st.session_state['df'].drop(target, axis=1)
    
    ignore_list= st.multiselect("Select columns to ignore: ",st.session_state['df'].columns)
