...
if st.button("Alrighty this is the moment of truth - let's punch those numbers and predict with our model!", type='primary') == True:
    ...
    accuracy_score = new_prediction['prediction_label'].sum() /new_prediction['Target_Variable_Actual'].sum()
    holdout_df_size= new_prediction['Target_Variable_Actual'].count()
    st.write(f'Accuracy of the model is:{accuracy_score*100}% from our holdout dataset {holdout_df_size} rows')
