...
#try except block to handle errors if the dataset is too small or has other issues
    ...
    st.write('Beep bop beep bop  .... pop the kettle on :teapot: this may take a couple mins as my cat crunches those numbers and builds you a model!')
    ...
    st.subheader(':rainbow[Well done! You have trained your machine learning model, now you are ready to use it! Go to the navigation to continue.]:point_left:')
...
expander= st.expander('Scrolled down and you got a  little lost? Click here for more info')
expander.subheader("1. OK so far- you have uploaded a dataset and profiled the now, what next?")
expander.write('Now we train a machine learning model to predict a target variable. This is where the magic happens! Go to the top and follow the instructions to train your model.')
expander.subheader("2. How does it work?")
expander.write('Depending on ML Algorithm, the model will learn from historical data to create a set of complex rules, this is your model. We then can use this model to make predictions on new data.')

expander.subheader("3. How are we doing this in this app?")
expander.write('We are using Pycaret, an AutoML library that trains and compares multiple machine learning models. Pycaret will train 10+ models and compare them to find the best model for your data. ')
    
expander.subheader("4. What about cleaning my data? It's a mess!")
expander.write('Pycaret will handle missing values, encoding, scaling, and other data preprocessing steps for you. It will also handle class imbalance and multicollinearity.') 
